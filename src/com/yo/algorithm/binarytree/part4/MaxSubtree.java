...
public class MaxSubtree {
    ...
    public TreeNode  postOrder(TreeNode root, int[] ret) {
        ...
        ret[0] = Math.max(rMax, root.value);//更新最大值
        ret[1] = Math.min(lMin, root.value);

        if( lNode==root.left && rNode==root.right
                && lMax<root.value && rMin>root.value ) {
            ret[2] = rNum + lNum + 1;
            return root;
        }
    ...
    public static TreeNode getMaxSearchSubTree(TreeNode root) {
        if( root == null ) {
            return null;
        }
        return preOrderTest(root, new int[3]);
    }
    public static TreeNode preOrderTest(TreeNode root, int[] ret) {
        if( root == null ) {
            ret[0] = Integer.MIN_VALUE;
            ret[1] = Integer.MAX_VALUE;
            ret[2] = 0;
        ...
        TreeNode lNode = preOrderTest(root.left, ret);
        int lMax = ret[0];
        int lMin = ret[1];
        int lNum = ret[2];
        TreeNode rNode = preOrderTest(root.right, ret);
        int rMax = ret[0];
        int rMin = ret[1];
        int rNum = ret[2];

        ret[0] = Math.max(rMax, root.value);
        ret[1] = Math.min(lMin, root.value);

        if( lNode==root.left && rNode==root.right && lMax<root.value && rMin>root.value ) {
            ret[2] = rNum + lNum + 1;
            return root;
        }
        ret[2] = Math.max(lNum, rNum);
        return rNum>=lMax ? rNode : lNode;
    }
}
